...
# ibm.com/rocketaihub-operator-bundle:$VERSION and ibm.com/rocketaihub-operator-catalog:$VERSION.
OPERATOR_NAME ?= rocketaihub-operator
IMAGE_TAG_BASE ?= $(REGISTRY_HOST)/$(REGISTRY_ORG)/$(OPERATOR_NAME)
...
.PHONY: get-sources
get-sources: # get all the sources used in the build
	./get_sources.sh

.PHONY: delete-sources
delete-sources: # get all the sources used in the build
	rm -rf gpu-operator kubeflow-ppc64le-manifests
...
.PHONY: docker-buildx
docker-buildx: get-sources ## Build and push docker image for the manager for cross-platform support
...
catalog-buildx: catalog ## Build a catalog image.
	$(CONTAINER_TOOL) buildx create --name project-v3-builder
	$(CONTAINER_TOOL) buildx use project-v3-builder
